...
ggPenguin0 <- ggplot(penguins,
       aes(x=flipper_length_mm, y=body_mass_g)) +
  geom_point() +
  labs(caption="Horst, Hill & Gorman (2020)",
       x="Flipper length (mm)",
       y="Body mass (g)") +
  theme_minimal()
...
ggplotly(ggPenguin0)  # reminder: tool tip displays variables mapped in aes() by default
...
ggplotly(ggPenguin,tooltip="text")
...
gTrill <- ggplot(GDPcombo, 
                 aes(x=year,
                     y=TotalGDPTrill,
                     fill=order_continent,
                     text=paste0("GDP = $",
                                format(TotalGDPTrill,digits=1),
                                " Trillion"))) + 
  geom_col() +
  coord_cartesian(expand=FALSE)

ggplotly(gTrill, tooltip="text")
